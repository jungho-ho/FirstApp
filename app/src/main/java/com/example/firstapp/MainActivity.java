...
public class MainActivity extends AppCompatActivity {
    ...
    private void calcu_수익률(){
        ...
        double 수익률 = 년수익 / (double)(Integer.parseInt(투자금.getText().toString())) * 100;
        EditText dd = (EditText) findViewById(R.id.editTxt_7);
        dd.setText(String.format("%.0f", 수익률));//7.수익률
    }

    private void calcu_대출이자(){
        EditText 대출원금 = (EditText) findViewById(R.id.editTxt_2);
        EditText 금리 = (EditText) findViewById(R.id.editTxt_3);

        double 년이자 = (Integer.parseInt(대출원금.getText().toString()))*((double)(Integer.parseInt(금리.getText().toString())) / (double)100);
